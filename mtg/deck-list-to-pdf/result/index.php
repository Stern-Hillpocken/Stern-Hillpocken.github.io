...
/', $_POST['text']);
        ...
        for($i = 0; $i < count($cardName); $i ++){
          if($_POST['lang'] == "en"){
            ...
            curl_setopt($curl_handle, CURLOPT_URL,'https://api.scryfall.com/cards/search?q=name:%22'.strtolower($cardName[$i]).'%22');
            ...
            $curl_handle=curl_init();
            curl_setopt($curl_handle, CURLOPT_URL,'https://api.magicthegathering.io/v1/cards?name='.preg_replace('/ /', '%20', $cardName[$i]));
            curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
            curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Your application name');
            $query = curl_exec($curl_handle);
